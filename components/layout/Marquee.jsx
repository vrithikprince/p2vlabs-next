...
export default function Marquee() {
  ...
  return (
    <div className="overflow-hidden py-5 border-y border-amp-hairline select-none bg-white"
      ...
      <div ref={trackRef} style={{ display: 'inline-flex', whiteSpace: 'nowrap', willChange: 'transform' }}>
        {all.map((t, i) => (
          <span key={i} style={{ display: 'inline-flex', alignItems: 'center', paddingRight: '3.5rem' }}>
            <span className="text-[11px] tracking-[0.3em] uppercase font-medium text-amp-caption">{t}</span>
            <span className="ml-14 text-amp-navy" style={{ fontSize: '0.45rem', lineHeight: 1 }}>◆</span>
          </span>
        ))}
      </div>
    </div>
