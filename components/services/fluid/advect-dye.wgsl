...
fn main(@builtin(global_invocation_id) id: vec3u) {
  ...
  // P2V: idle emitter colours retuned to the amp palette. Upstream pairs
  // azure with hot magenta (1.0, 0.08, 0.55); the magenta fought the navy
  // band badly. Now periwinkle #6980ff and violet #a273ff, the two accents
  // this band already uses. Weights and everything else are untouched.
  color += vec4f(0.41, 0.50, 1.0, 1.0) * emitter_weight(p, input.idle_a, aspect) * 0.12;
  color += vec4f(0.64, 0.45, 1.0, 1.0) * emitter_weight(p, input.idle_b, aspect) * 0.115;
