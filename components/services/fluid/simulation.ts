...
function inputUniforms(fluid: Fluid, input?: StirInput) {
  ...
  return {
    ...
    // P2V: splat colour still comes from movement direction with blue held
    // high, exactly as the reference does - but the sweep is remapped from
    // cyan/magenta onto periwinkle #6980ff to violet #a273ff so a stir stays
    // inside the band's palette. Direction still drives it; only the two
    // endpoints changed.
    pointer_color: [
      0.41 + 0.23 * (0.5 + 0.5 * direction[0]!),
      0.50 - 0.05 * (0.5 + 0.5 * direction[1]!),
      1,
      1,
    ],
