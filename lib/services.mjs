...
/* Index numerals are a design element on these pages the way they are on the
   homepage cards - assigned here so the hub, the detail page and the ghost
   numeral behind it can never disagree. */
SERVICES.forEach((s, i) => { s.n = String(i + 1).padStart(2, '0') })
...
/**
 * `rgb` is an unbracketed "R G B" triple, not a second copy of `hex`.
 * It feeds `rgb(var(--svc-accent) / 0.1)` so a single custom property
 * drives a row's rule, its gradient sweep and its glow, instead of three
 * near-identical colour strings that drift apart the first time an accent
 * is retuned.
 */
export const ACCENT = {
  navy:       { text: 'text-amp-navy',       bg: 'bg-amp-navy',       dot: 'bg-amp-navy',       hex: '#001a4f', rgb: '0 26 79' },
  violet:     { text: 'text-amp-violet',     bg: 'bg-amp-violet',     dot: 'bg-amp-violet',     hex: '#a273ff', rgb: '162 115 255' },
  periwinkle: { text: 'text-amp-periwinkle', bg: 'bg-amp-periwinkle', dot: 'bg-amp-periwinkle', hex: '#6980ff', rgb: '105 128 255' },
}
